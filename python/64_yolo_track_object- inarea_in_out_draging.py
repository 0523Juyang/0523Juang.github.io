...
# 用於追蹤的列表，key 是物件 ID
#objecttrackList = defaultdict(lambda: []) # Store tracking ID history, key is the object ID
...
while True:
    try:
        ...
        if len(area[0]) == 4: 
            ...
            for data in result_list:
                ...
                if results[0].boxes.id is not None:  # 檢查是否有檢測到物件分數
                    if int(data[6]) in [0,2,5,7]:  # 如果檢測到的物件是object, object ID = 0
                        #print(f"Detect ID: {str(data[4])}")
                        
                        x1, y1, x2, y2 = int(data[0]), int(data[1]), int(data[2]), int(data[3])  # 取得object的邊界框座標                  
                        object_center = ((int((x1 + x2) / 2), int((y1 + y2) / 2)))
                        objectIn = inArea(object_center, area) # 計算object在哪一區域
                        
                        trackList = pointTrackList[int(data[4])]# 取得該區域物件座標的追蹤列表
                        

                        if objectIn >= 0:  # 如果 object 在區域內
    
                        # 將object的中心點加入追蹤列表
                            trackList.append(object_center)
                        # 繪製追蹤路徑
                            ...
                            if len(pointTrackList[int(data[4])]) > 10:
                                ...
                                if prev_y>= area[objectIn][0][1] and ((curr_y -prev_y) >sense):
                                    if (int(data[4]) not in out_countList[objectIn]) and (int(data[4]) not in in_countList[objectIn]):
                                        out_countList[objectIn].append(int(data[4]))  # 儲存離開ID
                                elif prev_y <= area[objectIn][3][1] and ((prev_y - curr_y) >sense) :
                                    if (int(data[4]) not in out_countList[objectIn]) and (int(data[4]) not in in_countList[objectIn]):
                                        in_countList[objectIn].append(int(data[4]))  # 儲存進入ID
                                # 在影像上繪製object的邊界框和 ID
        ...
        # 顯示目前追蹤的object數量
        cv2.putText(frame, 'Line_1 Up:' + str(len(in_countList[0]))+' Down:'+str(len(out_countList[0])), (30, 200), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 255), 3, cv2.LINE_AA)
