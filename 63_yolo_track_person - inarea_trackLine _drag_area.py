...
'''
area = [
    [[1206, 189], [1257, 571], [1062, 578], [1043, 208]],   # 區域1
    [[1485, 185], [1696, 167], [1881, 555], [1733, 580]]    # 區域2
    ]
'''
...
while True:
    ...
    # 繪製已完成的線條
            ...
            for data in result_list:
                ...
                if results[0].boxes.id is not None:  # 檢查是否有檢測到物件分數
                    if int(data[6]) in [0]:  # 如果檢測到的物件是person, object ID = 0
                        ...
                        trackId = persontrackList[personIn]
                        trackList = pointTrackList[int(data[4])]
        ...
        # 顯示目前追蹤的person數量
        cv2.putText(frame, 'Line_1=' + str(len(persontrackList[0])), (30, 200), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 255), 3, cv2.LINE_AA)
        cv2.putText(frame, 'Line_2=' + str(len(persontrackList[1])), (30, 230), cv2.FONT_HERSHEY_PLAIN, 2, (255, 0, 255), 3, cv2.LINE_AA)

        # 顯示處理後的影像
        cv2.imshow('YOLOv8', frame)
        cv2.setMouseCallback('YOLOv8', mousePoints)  # 設置滑鼠回調函數
